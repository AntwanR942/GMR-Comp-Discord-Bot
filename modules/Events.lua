	...
	if Args[1] then
		if Args[1]:sub(1, #Prefix) == Prefix then
			...
			if Command then
				...
				if not Payload.member:hasPermission(0x00000020) then
					Log(2, "A user just entered a command they are not allowed to enter.")

					return SimpleEmbed(Payload, F("%s **%s**", Payload.author.mentionString, "you do not have permission to use that command."))
				end
